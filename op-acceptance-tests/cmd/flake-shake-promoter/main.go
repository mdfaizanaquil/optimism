...
func main() {
	...
	// Remove promotion candidates from flake-shake only; do not modify target gates in-file
	...
	updatedYAMLBytes, err := updateFlakeShakeGateOnly(originalYAML, gateID, promoteKeys)
	if err != nil {
		...
		if strings.HasPrefix(strings.TrimSpace(line), "tests:") && indentTests == "" {
			...
			for ; i < len(lines); i++ {
				...
				if !strings.HasPrefix(cur, indentGate+"  ") || strings.HasPrefix(strings.TrimSpace(cur), "- id:") {
					// step back one to reprocess in outer loop
					i--
					break
				}
